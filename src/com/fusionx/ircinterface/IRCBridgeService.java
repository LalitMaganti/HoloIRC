...
import com.fusionx.ircinterface.connection.ConnectionManager;
import com.fusionx.ircinterface.connection.ConnectionWrapper;
...
public class IRCBridgeService extends Service {
    ...
    public void connectToServer(final ServerConfiguration.Builder server,
        ...
        final BroadcastSender broadcastSender = new BroadcastSender(manager, this);
        final ConnectionWrapper thread = new ConnectionWrapper(configuration, this, broadcastSender);
        ...
        final AsyncTask<Void, Void, Void> disconnectTask = new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(final Void... strings) {
                ...
                if (status.equals(getString(R.string.status_connected))) {
                    server.getWriter().quitServer(Utils.getQuitReason(getApplicationContext()));
                } else {
                    connectionManager.get(serverName).interrupt();
                }
                return null;
            }
